...
void unconcious()
{
	...
	/*
	if(query_temp("no_dead")==1)
	{   remove_ride();
	    message_vision(HIW"只见$N化作一道青烟冲上天去。\n"NOR,this_object());
		this_object()->move("/d/lingtai/guanjin");
	    if( userp(this_object()) ) {
		set("gin", 1);	set("eff_gin", 1);
		set("kee", 1);	set("eff_kee", 1);
		set("sen", 1);	set("eff_sen", 1);
		set_temp("no_dead",0);}
		tell_object(environment(),HIW"一道青烟飘来慢慢凝成一个人。\n"NOR);
		return;}
	*/
...
void die()
{
	...
	object *inv;
	int i;
	string last_fainted_from, str,start;
	...
	if(query_temp("no_dead")==1 && query_skill_mapped("spells")=="dao")
	{   remove_ride();
	    message_vision(HIC"\n只见一根烂木头倒下，$N化作一道青烟冲上天去。\n"NOR,this_object());
		if(environment()->query("short") != "擂台" || environment()->query("short") != "禹王林") 
		{start=query("startroom");
		if(!start) start="/d/city/kezhan";
		move(start);
		}
	    if( userp(this_object()) ) {
		set("gin", 100);	set("eff_gin", 100);
		set("kee", 100);	set("eff_kee", 100);
		set("sen", 100);	set("eff_sen", 100);
		set_temp("no_dead",0);}
		tell_object(environment(),HIC"\n一道青烟飘来慢慢凝成一个人。\n"NOR);
		return;}
