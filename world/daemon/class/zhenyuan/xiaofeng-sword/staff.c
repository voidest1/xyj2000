...
void end_pfm_xfs(object me, int key,int enhance,int damage,object weapon);
...
me->set_temp("perform/key", perform_key+1);
 ...
 //    weapon->set("no_sell", "物品规格有误！！\n");
  ...
  enhance=me->query_skill("sword")/3;
    ...
    call_out("end_pfm_xfs",2,me,perform_key,enhance,damage,weapon);
...
void remove_effect(object me,int key,int enhance,int damage,object weapon) {
    ...
    end_pfm_xfs(me,key,enhance,damage,weapon);
}
...
void end_pfm_xfs(object me, int key,int enhance,int damage,object weapon)
{
    if (me->query_temp("wzg-powerup") &&
        me->query_temp("weapon")==weapon) {
          call_out("end_pfm_xfs",2,me,key,enhance,damage,weapon);
          return;
    }
