...
void end_pfm_sqjh(object me, int key, int enhance,int damage,object weapon);
...
me->set_temp("perform/key", perform_key+1);
  ...
  enhance=me->query_skill("sword")/3;
    ...
    call_out("end_pfm_sqjh",2,me,perform_key,enhance,damage,weapon);
...
void remove_effect(object me,int key,int enhance,int damage,object weapon) {
    ...
    end_pfm_sqjh(me,key,enhance,damage,weapon);
}
...
void end_pfm_sqjh(object me, int key,int enhance,int damage,object weapon)
{
    if (me->query_temp("wzg-powerup") &&
        me->query_temp("weapon")==weapon) {
          call_out("end_pfm",2,me,key,enhance,damage,weapon);
          return;
    }
