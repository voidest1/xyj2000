...
void create()
{
	set_name(HIW "玄冰刀" NOR, ({"xuanbing blade", "blade", "dao"}));
	set_weight(5000);
	set("unit", "把");
	set("long", "一把寒气逼人的弯刀，以大雪山中千年不化的玄冰制成。\n");
	set("value", 8);
	set("material", "ice");
	set("wield_msg",
...
void melt()
{
	...
	{//a player or a NPC.
		...
		{
			if(environment(env))
				message_vision("$N觉得身上凉凉的，湿湿的，原来是玄冰刀化了。\n", env);
	...
	{//a room.
		...
		{
			tell_object(env,"玄冰刀终于化成了一滩水，流得到处都是。\n");
			destruct(this_object());
			return;
		}
	}
...
int do_duandao(string arg)
{
	...
	if( ! arg )
	{
		message_vision(HIW "$N运力一震，只见冰花四溅！一把玄冰刀顿时化为无数碎片。\n" NOR, me);
		me->start_busy(3);
