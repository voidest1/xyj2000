...
#include "/d/dntg/yunlou/npc/yaoguai_equip.c";
		...
		message_vision("$N死了。\n",this_object());
			...
			".\n";
                    if (owner_ob->query_temp("mieyao/level")==9 && ratio>50) {
					   ...
					   eq_level=give_equip(owner_ob,me);
					   str+= "and rewarded 1 lvl of "+skill+" and "+eq_level+".\n";
                    }
			...
			if(owner_ob->query("mieyao/npcid"))
			{
				base = 1;
				if(owner_ob->query("mieyao/search") > 0)
				{
					if(owner_ob->query("mieyao/search") >= 5)
						base = base * pow(to_float(10), 5);
					else
						base = base * pow(to_float(10), to_float(owner_ob->query("mieyao/search")));
				}
				else
					owner_ob->set("mieyao/search", 0);

				questnpc = find_object(owner_ob->query("mieyao/npcid"));
				if((owner_ob->query_temp("mieyao/level")==9 && owner_ob->query("mieyao/search")==0)
					|| (questnpc && random(base) == random(base))
                  )
				{
					owner_ob->add("mieyao/search", 1);
					tell_object(owner_ob, HIG+"你获得了一次使用"+HIW+"照妖镜（ask li jing about search）"+HIG+"的机会，目前共"+owner_ob->query("mieyao/search")+"次"+NOR+"\n");
					// questnpc->command("tell "+owner_ob->query("id")+" ");
					owner_ob->delete("mieyao/npcid");
				}
			}
        } else {
	    message_vision("$N死了。\n",this_object());
