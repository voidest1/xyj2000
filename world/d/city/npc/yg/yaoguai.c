	...
	object questnpc;
        ...
        if( stringp(owner) && objectp(owner_ob = find_player(owner)) ) {
			...
			if(owner_ob->query("mieyao/npcid1"))
			{
				...
				questnpc = find_object(owner_ob->query("mieyao/npcid1"));
				if(questnpc && random(base) == random(base))
				{
					owner_ob->add("mieyao/search", 1);
					tell_object(owner_ob, HIG+"你获得了一次使用"+HIW+"五鬼搬运（ask yuan tiangang about search）"+HIG+"的机会，目前共"+owner_ob->query("mieyao/search")+"次"+NOR+"\n");
					//questnpc->command("tell "+owner_ob->query("id")+" 下次若寻不到妖魔的踪迹，可请我算上一卦（search）");
