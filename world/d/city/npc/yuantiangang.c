...
int relocate_me()
{
	...
	if( (t=(int)me->query("mieyao/time_start1")) &&
		...
		if( !ghost || (t<=time() && time()>=t+1800)) {
			message_vision("$N掐指一算，对$n说道：\n",
				this_object(),me);
			message_vision("　　"+me->query("mieyao/name1")+"不知所踪，"+RANK_D->query_respect(me)+"还是休息一会吧\n"
				,this_object(),me);
			return 1;
		}
		...
		{
			command("whisper "+me->query("id")+" 你还有"+me->query("mieyao/search")+"次算卦机会");
		}
		return 1;
